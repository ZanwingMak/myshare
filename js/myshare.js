...
var myshare = function (elementNode, config) {
    ...
    var isqqBrowser = (UA.split("mqqbrowser/").length > 1 && !(UA.match(/Mobile MQQBrowser/i) == "mobile mqqbrowser")) ? bLevel.qq.higher : bLevel.qq.forbid;
    ...
    this.appShowType = config.appShowType || [];
    this.defaultShowType = config.defaultShowType || 6;
    this.ucAppList = {
                ...
                case "weixinFriend" :
                    if(showType == 5 || showType == 6 || showType == 7){
                        $('#myshare').slideUp(function () {
                            //alert(_this.UpRightTipsType);
                            if(showType == 7){
                                showRightUpTips();
                            }
                            $('.intro').text('浏览器不支持，可长按下方链接，复制给好友：');
                            $('#myshare .myshare').hide();
                            $('#myshare').slideDown();
                        });
                    }else{
                        alert('当前浏览器可能不支持，还是直接复制下方的链接发给朋友吧~');
                    }
                    break;
                case "sinaWeibo" :
                    var wbLink = 'http://v.t.sina.com.cn/share/share.php?title='+ title +'&desc='+desc+'&url='+url+'&content=utf-8&sourceUrl='+url+'&pic='+img;
                    window.open(wbLink);
                    break;
                case "QQ" :
                    var qqLink = 'http://connect.qq.com/widget/shareqq/index.html?url=' + url + '&title=' + title + '&description=' + '' + '&charset=utf-8';
                    window.open(qqLink);
                    break;
                case "QZone" :
                    var qzLink = 'http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?site=www.tuan2.com&title=' + title + '&desc=' + desc + '&summary=' + '分享' + '&url=' + url + 'pic' + img;
                    window.open(qzLink);
                    break;
                case "QRcode" :
                    closeShare();
    ...
    this.bindEvents = function() {
        ...
        var layer_shade = document.querySelector('.layer-shade');
        $('body').on('touchstart','.qrcode-close',function(){
            //关闭二维码
            $('.qrcode-mask').fadeOut(function(){
                $('.qrcode-mask').remove();
            });
        });
        if(close){
        ...
        document.getElementById(elementNode).addEventListener('click', function () {
            $('#myshare').hide();
            document.getElementById('myshareBox').style.display = 'block';
            $('#myshare').slideDown(function () {
            });
        });
    }
    ...
    //右上角提示
    function appendRightUpTips() {
        $('#myshareBox').append('<div class="wxshare"></div>');
    }
    function removeRightUpTips() {
        $('.wxshare').remove();
    }
    function showRightUpTips() {
        $('.wxshare').show();
    }
    function hideRightUpTips() {
        $('.wxshare').hide();
    }
    ...
    this.init = function () {
        ...
        this.isloadqqApi();
        if ((isqqBrowser || isucBrowser) && !isWeixin && !isInnerQQ) {
            showType = 2;
            this.html(showType);
        } else if (isWeixin) {
            ...
            for(var i=0;i<_3rdAppKeys.length;i++){
                if( (_3rdAppUA.indexOf(_3rdAppKeys[i])) > -1 ){
                    ...
                    this.html(_3rdAppVals[i]);
                    break;
                }
            }
            showType = this.defaultShowType;
            this.html(showType);
        }
    };
    ...
    var share = this;
    var items = document.getElementsByClassName('myshare');
